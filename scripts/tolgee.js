...
const downloadFiles = async (client, languages, filterNamespace) => {
  try {
    const a = await client.post(`/projects/export`, { zip: true, format: "JSON", languages, filterNamespace }, { responseType: 'arraybuffer' });
    
    const zip = await fromBuffer(Buffer.from(a.data), ZIP_OPTIONS);
    unzip(zip, path.join(__dirname, "../src/locale/translations/downloaded"));
  } catch (e) {
    console.log(e);
  }
}
