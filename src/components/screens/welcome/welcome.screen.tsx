...
const WelcomeScreen: SFC<IProps> = ({ onSignUpPress, onLogInPress }) => (
    <CentredScreen footerImage="forest">
        <Pad height={100} />
        <Heading size={Heading.Sizes.LARGE} label={data.heading} />
        <Blurb label={data.blurb} />
        <Pad height={44} />
        <Button type={Button.Types.SECONDARY} label={data.signUpCta} onPress={onSignUpPress} />
        <Pad height={15} />
        <Button type={Button.Types.SECONDARY} label={data.logInCta} onPress={onLogInPress} />
    </CentredScreen>
);
