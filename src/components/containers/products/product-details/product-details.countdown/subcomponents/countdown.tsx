...
import { getCountdownFromDate } from "../../utils/getCountdownFromDate";
...
export const Countdown = memo(({ secondsUntilStartDate, countdownEndCallback }: Props) => {
  const { days, hours, minutes } = useCountdownHandler(secondsUntilStartDate, countdownEndCallback);
...
const useCountdownHandler = (secondsDiff: number, countdownEndCallback: () => void) => {
  const secondsDiffRef = useRef(secondsDiff);
  const [countdown, setCountdown] = useState(getCountdownFromDate(secondsDiff));
  ...
  useEffect(() => {
    recountTimer.current = setInterval(() => {
      ...
      setCountdown(getCountdownFromDate(secondsDiffRef.current));
