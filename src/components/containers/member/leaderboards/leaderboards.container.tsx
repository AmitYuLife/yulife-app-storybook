...
const LeaderboardsContainer: FC<Props> = ({
    ...
    const [{ activeLeaderboardIndex, leaderboardId }, setLeaderboardInfo] = useState(
        getInitialLeaderboard(leaderboards)
    );
    ...
    const handleLeaderboardChange = useCallback(
        (index: number) => {
            ...
            Logger.logEvent("screen_view", {
                name:
                    newId.length === 32 ? "yulife.member.Leaderboards.Primary" : "yulife.member.Leaderboards.Secondary",
