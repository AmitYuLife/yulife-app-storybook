...
import { TITLE_Y_POSITION } from "./step-feedback-constants";
...
export const useStepFeedbackAnimations = () => {
  ...
  const scrollHandler = useAnimatedScrollHandler((event) => {
    ...
    scrollPercentage.value = clamp(interpolate(event.contentOffset.y, [0, -HEADER_TOP_CONTAINER_MAX_Y], [0, 1]), -1, 1);
  ...
  const rewardContainerStyle = useAnimatedStyle(() => {
    return {
      opacity: interpolate(scrollPercentage.value, [0, 0.7, 1], [1, 1, 0]),
      transform: [
        {
