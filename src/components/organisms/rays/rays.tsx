  ...
  return (
    <Animated.View style={wrapperStyle}>
      <Animated.View style={animatedStyle}>
        {style === "default" ? <PodiumRaysSvg gradientStops={gradientStops} /> : null}
        {style === "alternate" ? <RaysSvgAlternate color={color} gradientStops={gradientStops} /> : null}
        {style === "thin" ? <RaysSvgThin color={color} gradientStops={gradientStops} /> : null}
      </Animated.View>
    </Animated.View>
  );
};
...
const renderStops = (stops: IGradientStop[]) =>
  stops.map((stop, index) => (
    <Stop key={index} stopColor={stop.color} offset={stop.offset} stopOpacity={stop.opacity ?? 1} />
  ));

const PODIUM_DEFAULT_STOPS: IGradientStop[] = [
  { color: "#fff", offset: 0, opacity: 0 },
  { color: "#fff", offset: 0.661458 },
];

const PodiumRaysSvg = memo(({ gradientStops }: ISvgProps) => {
  const stops = renderStops(gradientStops ?? PODIUM_DEFAULT_STOPS);
...
const RaysSvgThin = memo(({ color = "#fff", gradientStops }: ISvgProps) => {
...
const RaysSvgAlternate = memo(({ color = "#fff", gradientStops }: ISvgProps) => {
  const stops = renderStops(
    gradientStops ?? [
      { color, offset: 0, opacity: 0 },
      { color, offset: 1 },
