...
describe("Onboarding Saga: onboardOnGetUser", () => {
    it("does nothing if already onboarded and sent historical data", () => {
        ...
        expect(saga.next({ usePassiveMeditation: true }).value).toEqual(select(getIsHistoricalDataCollected));
        expect(saga.next(true).value).toEqual(select(getIsHistoricalMeditationDataCollected));
    ...
    it("sends historical data and redeems onboarding", () => {
        ...
        expect(saga.next().value).toEqual(select(getUserFeatures));
        expect(saga.next({ usePassiveMeditation: true }).value).toEqual(select(getIsHistoricalDataCollected));
        expect(saga.next(false).value).toEqual(select(getIsHistoricalMeditationDataCollected));
        ...
        expect(saga.next().value).toEqual(call(sendHistoricalMeditationData, moment("2019-07-07T12:12:12")));
