import deepLink from "@navigation/deepLink";
import { getRouteState } from "@redux/app/app.selectors";
import { getToken } from "@services/storage";
import { Unpacked } from "@utils";
import { call, select, take } from "redux-saga/effects";
import { linkingChannel } from "../app.channels";

export default function* listenToDeepLinkingSaga() {
  const stateChannel: ReturnType<typeof linkingChannel> = yield call(linkingChannel);

  while (true) {
    const url: { url: string } = yield take(stateChannel);
    const token: Unpacked<typeof getToken> = yield call(getToken);

    if (url?.url) {
      const currentRoute: string = yield select(getRouteState);
      yield call(deepLink.init, url.url, !!token, currentRoute);
    }
  }
}
