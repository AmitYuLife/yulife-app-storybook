...
export const INTRO_START = async (customer = CUSTOMER_37) => {
  ...
  When("I scroll down the page", when.swipeFromText("Power up!", "up", "fast"), async () => {
    ...
    When("I tap Get started button", when.tapText("Get started"), async () => {
      Then(
        "I should be on the Let's get personal screen",
        then.isOnLetsGetPersonalScreen(`${customer.data.firstName}`)
      );
    });
  });
};
...
export const UNDERWRITING_SALARY = async (salary: string) => {
  ...
  When(
    "I choose the correct date of birth",
    [when.typeViaID(CONTENT_ITEM_INPUT("salary"), salary), when.tapText("Enter your salary")],
    async () => {
...
export const UNDERWRITING_CITIZEN = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
    Then(
      "I should be on the hazardous employment screen",
      then.isOnScreen("Are you employed in any of the following occupations?")
    );
  });
};
...
export const UNDERWRITING_EMPLOYMENT = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
    Then(
      "I should be on the height screen",
      then.isOnScreen("Getting a bit more personal now... I’m a whopping 16ft 4in tall, what about you?")
    );
  });
};
...
export const UNDERWRITING_HEIGHT = async (answer: string) => {
  When(`I tap on Enter your height`, when.tapText("Please enter your height"), async () => {
    ...
    When(
      `I choose ${answer}`,
      when.swipeOnPicker(SCROLL_PICKER("heightCm"), SCROLL_PICKER_ACTIVE_ITEM(answer), "down"),
      async () => {
        ...
        When("I tap Select", when.tapText("Select"), async () => {
          Then(
            "I should be on the same screen",
            then.isOnScreen("Getting a bit more personal now... I’m a whopping 16ft 4in tall, what about you?")
          );
          Then("I should be back on the height screen", then.textVisible(answer.replace(" ", "")));
          When(
            "I tap Continue",
            when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "Continue", "down"),
            async () => {
...
export const UNDERWRITING_WEIGHT = async (answer: string) => {
  When(`I tap on Enter your weight`, when.tapText("Please enter your weight"), async () => {
    ...
    When(
      `I choose ${answer}`,
      when.swipeOnPicker(SCROLL_PICKER("weightKg"), SCROLL_PICKER_ACTIVE_ITEM(answer), "down"),
      async () => {
        Then(`I should have selected ${answer}`, then.idVisible(SCROLL_PICKER_ACTIVE_ITEM(answer)));
        When("I tap Select", when.tapText("Select"), async () => {
          ...
          When(
            ...
            async () => {
              Then(
                "I should be on the cigarretes screen",
                then.isOnScreen("When was the last time you smoked a cigarette?")
              );
            }
          );
        });
...
export const UNDERWRITING_CIGARETTES_QUANTITY = async (option: QuantityOption) => {
  When(`I tap ${option} and Continue`, when.selectOptionAndNavigate(option, "Continue"), async () => {
    Then(
      "I should be on the cigars screen",
      then.isOnScreen("When was the last time you smoked cigars, pipes or shisha?")
    );
  });
};
...
export const UNDERWRITING_CIGARS = async (option: TimeOption) => {
  When(`I tap ${option} and Continue`, when.selectOptionAndNavigate(option, "Continue"), async () => {
    Then(
      "I should be on the smoking alternatives screen",
      then.isOnScreen(
        "When was the last time you used e-cigarettes, vapes or another nicotine substitute such as nicotine gum or patches?"
      )
    );
  });
};
...
export const UNDERWRITING_ALCOHOL = async (answer: string) => {
  When(`I tap on Enter number of drinks`, when.tapText("Enter number of drinks"), async () => {
    ...
    When(
      `I choose ${answer}`,
      when.swipeOnPicker(SCROLL_PICKER("drinkQuantity"), SCROLL_PICKER_ACTIVE_ITEM(answer), "down"),
      async () => {
        Then(`I should have selected ${answer}`, then.idVisible(SCROLL_PICKER_ACTIVE_ITEM(answer)));
        When("I tap Select", when.tapText("Select"), async () => {
...
export const UNDERWRITING_CANNABIS_QUANTITY = async (option: QuantityOption) => {
  When(`I tap ${option} and Continue`, when.selectOptionAndNavigate(option, "Continue"), async () => {
    Then(
      "I should be on the recreational drugs screen",
      then.isOnScreen("When was the last time you used recreational drugs?")
    );
  });
};
...
export const UNDERWRITING_RECREATIONAL_DRUGS = async (option: TimeOption) => {
  When(`I tap ${option} and Continue`, when.selectOptionAndNavigate(option, "Continue"), async () => {
    Then(
      "I should be on the counselling screen",
      then.isOnScreen(
        "Have you ever been advised to receive and/or received treatment, counselling, or attended a support group to manage your alcohol or drug use?"
      )
    );
  });
};
...
export const UNDERWRITING_COUNSELLING = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
    Then(
      "I should be on the sex screen",
      then.isOnScreen("Now I’ll need a bit of your medical history. What is your sex?")
    );
  });
};
...
export const UNDERWRITING_SEX = async (sex: "Male" | "Female") => {
  When(`I tap ${sex} and Continue`, when.selectOptionAndNavigate(sex, "Continue"), async () => {
    Then(
      "I should be on the diagnosed with screen",
      then.isOnScreen("Have you ever been diagnosed with one of the following?")
    );
  });
};
...
export const UNDERWRITING_DIAGNOSED_WITH = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.scrollToTheBottomAndChooseYesOrNo("LEFT_PRODUCT_STEP_MULTI_BUTTON"), async () => {
    Then(
      "I should be on the diagnosed with screen",
      then.isOnScreen(
        "Have you required 3 or more consultations in the last 3 years from a healthcare professional for an individual medical or mental health condition, symptom, illness or injury?"
      )
    );
  });
};
...
export const UNDERWRITING_MANY_CONSULTATIONS = async (answer: YesNo) => {
  if (answer === "No") {
    When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
      Then(
        "I should be on the awaiting tests screen",
        then.isOnScreen("Are you awaiting any investigations/tests (including their results) or any surgery?")
      );
    });
  } else {
    When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
      Then(
        "I should be on the condition selection screen",
        then.isOnScreen("Select all the conditions for which you required consultations")
      );
    });
  }
};
...
export const UNDERWRITING_CONDITION_SELECTION = async (conditions: Condition[]) => {
  for (const condition of conditions) {
    When(
      `I scroll to ${condition}`,
      when.scrollUntilIdVisible(PRODUCT_STEP_BODY_SCROLL_VIEW, CONDITION_OPTION(condition, false), "down"),
      async () => {
  ...
  When("I tap continue", when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "Continue", "down"), async () => {
    Then(
      "I should be on the conditions confirmations screen",
      then.isOnScreen(`You’ve selected ${conditions.length.toString()} ${conditionText}.`)
    );
    When(
      `I confirm I have chosen ${conditionLength} ${conditionText}`,
      when.tapText("I confirm these are all the conditions"),
      async () => {
        Then(
          "I should be on the next screen",
          then.textNotVisible(`You’ve selected ${conditionLength} ${conditionText}.`)
        );
      }
    );
  });
...
export const UNDERWRITING_BP = async (answer: YesNo, choice: string) => {
  When(`I tap ${answer}`, when.tapText(answer), async () => {
    Then(
      "I should be on the BP checkup screen",
      then.isOnScreen("What were you told at your latest blood pressure checkup?")
    );
    When(`I tap ${choice}`, when.tapText(choice), async () => {
      When("I tap continue", when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "Continue", "down"), async () => {
        Then(
          "I should be on the next screen",
          then.textNotVisible("What were you told at your latest blood pressure checkup?")
        );
      });
    });
  });
...
export const UNDERWRITING_CHOLESTEROL = async (answer: YesNo, choice: string) => {
  When(`I tap ${answer}`, when.tapText(answer), async () => {
    Then(
      "I should be on the Cholesterol checkup screen",
      then.isOnScreen("What were you told at your latest cholesterol checkup?")
    );
    When(`I tap ${choice}`, when.tapText(choice), async () => {
      When("I tap continue", when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "Continue", "down"), async () => {
        Then(
          "I should be on the next screen",
          then.textNotVisible("What were you told at your latest cholesterol checkup?")
        );
      });
    });
  });
...
export const UNDERWRITIING_EARS_NOSE_THROAT = async (answer: YesNo) => {
  When(
    `I am on the Ears, nose, throat screen and tap ${answer}`,
    when.tapAnswerOnConditionScreen("Ears, nose, throat", answer),
    async () => {
      Then(
        "I should be on the next screen",
        then.textNotVisible("Were all of your ear, nose and throat issues in the list below?")
      );
    }
  );
};
...
export const UNDERWRITIING_DIGESTIVE = async (answer: YesNo) => {
  When(
    `I am on the Digestive screen and tap ${answer}`,
    when.tapAnswerOnConditionScreen("Digestive", answer),
    async () => {
      Then(
        "I should be on the next screen",
        then.textNotVisible("Were all of your ear, nose and throat issues in the list below?")
...
export const UNDERWRITIING_KIDNEYS_BLADDER = async (answer: YesNo) => {
  When(
    `I am on the Kidneys & bladder screen and tap ${answer}`,
    when.tapAnswerOnConditionScreen("Kidneys & bladder", answer),
    async () => {
      Then(
        "I should be on the next screen",
        then.textNotVisible("Were all of your ear, nose and throat issues in the list below?")
...
export const UNDERWRITIING_MINOR_INJURIES = async (answer: YesNo) => {
  When(
    `I am on the Minor injuries screen and tap ${answer}`,
    when.tapAnswerOnConditionScreen("Minor injuries", answer),
    async () => {
      Then(
        "I should be on the next screen",
        then.textNotVisible("Were all of your ear, nose and throat issues in the list below?")
...
export const UNDERWRITIING_MUSCLES_JOINTS = async (answer: YesNo) => {
  When(
    `I am on the Muscles & joints screen and tap ${answer}`,
    when.tapAnswerOnConditionScreen("Muscles & joints", answer),
    async () => {
      Then(
        "I should be on the next screen",
        then.textNotVisible("Were all of your ear, nose and throat issues in the list below?")
...
export const UNDERWRITING_MEDICAL_FOLLOW_UP = async (isRejected: boolean) => {
  if (!isRejected) {
    When(
      "I am on the overnight stay screen and tap No",
      [
        then.textVisible(
      ...
      async () => {
        Then(
          "I should be on the fully resolved symptom screen",
          then.textVisible("Are these conditions or symptoms fully resolved?")
        );
        When("I tap No", when.tapText("No"), async () => {
          Then(
            "I should be on the stable symptom screen",
            then.textVisible("Are these conditions stable (no increasing symptoms or complications)?")
          );
          When("I tap Yes", when.tapText("Yes"), async () => {
            Then(
              "I should be on the restrictions screen",
              then.textVisible("Do you have restrictions of your daily activities, tasks or occupation?")
            );
            When("I tap No", when.tapText("No"), async () => {
              Then(
                "I should be on the next screen",
                then.textNotVisible("Do you have restrictions of your daily activities, tasks or occupation?")
              );
            });
          });
        });
    ...
    When(
      "I am on the overnight stay screen and tap Yes",
      [
        then.textVisible(
          "For the issues you had that were not on the list, have you required an overnight stay in hospital in the last 2 years?"
        ),
...
export const UNDERWRITING_AWAITING_TESTS = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
    Then(
      "I should be on the 6 month symptom screen",
      then.isOnScreen(
        "In the last 6 months have you noticed any symptoms for which you have not yet received an exact diagnosis, that you continue to experience and for which you expect to consult a healthcare professional?"
      )
    );
  });
};
...
export const UNDERWRITING_SYMPTOMS = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
    Then(
      "I should be on the covid hospital",
      then.isOnScreen("In the last 12 months have you been hospitalised for COVID-19?")
    );
  });
};
...
export const UNDERWRITING_COVID_HOSPITAL = async (answer: YesNo) => {
  ...
  When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
    Then(
      "I should be on the covid exposure screen",
      then.isOnScreen("In the last 30 days have any of the following applied to you:")
    );
  });
};
...
export const UNDERWRITING_COVID_EXPOSURE = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.chooseYesOrNo(answer), async () => {
    Then(
      "I should be on the other policies screen",
      then.isOnScreen("Almost done! Do you currently hold any other personal life insurance policies?")
    );
  });
};
...
export const UNDERWRITING_DO_POLICIES_EXCEED = async (answer: YesNo) => {
  When(`I tap ${answer}`, when.tapText(answer), async () => {
    Then(
      "I should be on the review screen",
      then.isOnScreen("Please take a quick look over your answers before submitting.")
    );
  });
};
...
export const COVER_SELECT_PERCENTAGE = async (percentage: percentageLevel) => {
  ...
  When(
    "I scroll to the bottom and tap Continue to checkout",
    when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "Continue", "down"),
    async () => {
...
export const COVER_PRICE_CHECK = async (answer: string) => {
  ...
  When(`I choose 36`, when.swipeOnPicker(DATE_PICKER, SCROLL_PICKER_ACTIVE_ITEM("35"), "left"), async () => {
    Then(`I should have selected "35"`, then.idVisible(SCROLL_PICKER_ACTIVE_ITEM("35")));
    Then(
      `I should see corect Price changed according when i am  "35" years`,
      then.multiFactorPriceChange(30, "£4.94", "£1,041.67", 35)
    );
  });
  When(
    `I choose ${answer}`,
    when.swipeOnPicker(DATE_PICKER, SCROLL_PICKER_ACTIVE_ITEM(answer), "right", 200),
    async () => {
      Then(`I should have selected ${answer}`, then.idVisible(SCROLL_PICKER_ACTIVE_ITEM(answer)));
      Then(
        `I should see corect Price changed according when i am  ${answer} years`,
        then.multiFactorPriceChange(30, "£17.96", "£1,041.67", 70)
      );
    }
  );
  ...
  When(
    "I scroll and tap on Documents",
    when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "Documents", "down"),
    async () => {
...
export const COVER_STYLE_SELECTION = async (
  cover: coverLevel,
  percentage: string,
  mothprice: MonthlyCoverPrices,
  totalCoverPrices: TotalCoverPrices
) => {
...
export const MAXIMUM_SUM_ASSURED = async (
  ...
  When(
    "I am on maximum sum assured screen and tap View full details",
    [
      then.isOnScreen("Based on the information you've given us, this is the maximum coverage available."),
      when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "View full details", "down"),
    ],
    async () => {
      ...
      When(`I choose 31 %`, when.swipeOnPicker(HORIZONTAL_SCROLLER, SCROLL_NUMBER_PICKER(29), "left"), async () => {
        Then(
          "I should see correct price package appearing",
          then.multiFactorPriceChange(33, mothprice, totalCoverPrices, 60)
        );
        When(
          "I scroll to the bottom and tap Continue to checkout",
          when.scrollToAndTapText(PRODUCT_STEP_BODY_SCROLL_VIEW, "Continue", "down"),
          async () => {
            ...
            When(
              ...
              async () => {
                Then(
                  "I should see correct package selected",
                  then.packageSummaryVisible(cover, mothprice, totalCoverPrices)
                );
              }
            );
          }
...
export const FAST_INTRO_START = async () => {
  When("I tap Get started button", when.tapText("Get started"), async () => {
    When(
      "I scroll to the bottom",
      when.scrollUntilTextVisible(PRODUCT_STEP_BODY_SCROLL_VIEW, "Terms of Business", "down"),
      async () => {
        When("I tap Let's go!", when.tapText("Let's go!"), async () => {
          Then("I should be on promise Yugi screen", then.isOnPromiseYugiScreen);
          When("I tap Yes, I promise", when.tapText("Yes, I promise"), async () => {
...
export const FAST_CHECKOUT = async (isCovered: boolean, cover: string) => {
  ...
  When("I continue on and click through the contact details screen", when.scrollDownOnContactsPage, async () => {
    ...
    When("I click continue", when.tapText("Continue"), async () => {
      Then(
        "I should be on the next checkout page",
        then.textVisible("We may need to request a medical report from your doctor.")
      );
      When("I add GP details", when.addGPDetails, async () => {
        Then("I should be on the checkout page", then.isOnScreen("Checkout"));
        ...
        When("I tap purchase with existing card", when.tapText("Purchase with **** 4444"), async () => {
          Then(
            "I should see We've got you covered",
            then.textVisible("Item unlocked!\nYou've powered up your protection.")
          );
          Then(`I should see the ${nextScreenBody}`, then.isOnScreen(nextScreenBody));
        });
      });
