...
Feature("Smart Pension", async () => {
  Scenario("I can see an active connected pension", scenario.start, () => {
    Given("I login", given.loginAsUser(data.CUSTOMER_111, data.AUTH_111), async () => {
      When("I go to the YuCoin screen", when.navigateTo("yucoin"), async () => {
        Then("I should not see the contributions just yet", then.textNotVisible(`£${calculateInProgressContribution(PensionInfoUser111)}`, 5000));
      });
    });
    ...
    When("I swipe to the bottom", when.scrollFromID(ids.YUSCREEN, "up", "fast", 0.2, 4000), async () => {
  ...
  Scenario("I can see a pending pension due to contribution not exisiting", scenario.start, () => {
    ...
    When("I swipe to the bottom", when.scrollFromID(ids.YUSCREEN, "up", "slow", 0.1), async () => {
      When("I tap the pension slot", when.tapID(ids.YUSCREEN_V5_PRODUCT_INDIVIDUAL_CARD("Pension"), 3000), async () => {
  ...
  Scenario("I can see a pending pension due to contribution not having filled info", scenario.start, () => {
    Given("I login", given.loginAsUser(data.CUSTOMER_113, data.AUTH_113), async () => {
      When("I go to the YuCoin screen", when.navigateTo("yucoin"), async () => {
        Then("I am on the home page", then.idVisible(ids.VIEW_TOP_RIGHT_COIN_COUNTER(17700)));
      });
    });
  ...
  Scenario("I can see a paused pension due the contribution being set to paused", scenario.start, () => {
    Given("I login", given.loginAsUser(data.CUSTOMER_114, data.AUTH_114), async () => {
      When("I go to the Yu screen", when.navigateTo("yu"), async () => {
        Then("I do not see the onboarding screen as I have a connection", then.cannotSeePensionOnboarding);
        Then("I can see the slot has no icon", then.idNotVisible(ids.RIGHT_STATUS_ICON));
        Then("I can see the slot has no icon", then.idNotVisible(ids.LEFT_SIDE_BACKGROUD_IMAGE_SLOT(undefined)));
      });
    ...
    When("I tap manage", when.tapText("Manage", 4000), async () => {
