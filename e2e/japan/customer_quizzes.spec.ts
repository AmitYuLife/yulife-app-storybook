...
import { backSpace, hqInfoCopy, moneyMasteryFWQDescriptionPage, oneHundredAndOneCharacters, oneHundredCharacters, oneThousandAndOneCharacters } from "../activity/FWQ/_resources/fixtures";
...
Feature("Quizzes and questionnaires", async () => {
  Scenario("As a user that works at 1 company, I can traverse through the engagement survey until it is complete and I receive the correct amount of YuCoin.", scenario.start, async () => {
    ...
    When("I click on the 'Next' button", when.tapID(ids.BUTTON_BASE("次のページ")), async () => {
      Then("I should be on the 'What do you enjoy most about working at Biz' question", then.idVisible("TEXT_TEMPLATE_YU LIFE LTDでの仕事について、何が一番楽しいですか?h2"));
    });
    ...
    When("I click on the 'Next' button", when.tapID(ids.BUTTON_BASE("次のページ")), async () => {
      Then("I should be on the 'the one thing you would change about Biz' question", then.idVisible("TEXT_TEMPLATE_YU LIFE LTDでの体験をより良くするために、あなたが変えたいことは何ですか?h2"));
    });
    ...
    When("I click on the 'Submit' button", when.tapID(ids.BUTTON_BASE("送信")), async () => {
