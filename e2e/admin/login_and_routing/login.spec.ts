...
Feature("As a user I can get past the login screen", async () => {
  ...
  Scenario("I can login with correct login details and see the correct data for every passive/active activity that a user has engaged with", scenario.start, async () => {
    ...
    When("I tap on YuCoin", when.tapID(ids.DAILYSTEP_SCREEN_COIN, 4000), async () => {
    ...
    When("I scroll to the description if needed", when.scrollUntilTextVisible(ids.SETTINGS_SCREEN_SCROLL, "Fitness trackers", "down"), async () => {
      Then("I should see Measurement cycling title", then.idVisible(ids.TEXT_TEMPLATE("Measurement (Cycling)"), 2000));
    ...
    When("I tap the km measurement", when.tapID(ids.TEXT_TEMPLATE("km"), 4000), async () => {
    ...
    When("I select miles as measurement", when.tapID(ids.SETTINGS_NAME("mi"), 2000), async () => {
      When("I go close the settings screen", when.tapID(ids.BUTTON_CLOSE_HEADER("Settings"), 1500), async () => {
        Then("I should see 7.0 mi done today", then.idVisible(ids.CYCLING_COUNT("7.0 mi")));
        Then("I should Not see InfoIcon anymore", then.idNotVisible(ids.YUCOIN_POWER_INFO));
      });
    });
    When("I tap on YuCoin coin", when.tapID(ids.CYCLING_COUNT("7.0 mi"), 2000), async () => {
    ...
    When("I tap on the 'i' activity feed info tooltip", when.tapID(ids.ACTIVITY_FEED, 1500), async () => {
      Then("I should see 20 YuCoin for 1.0 mi cycling", then.textVisible("20 YuCoin for 1.0 mi cycling", 3000));
    });
  });
