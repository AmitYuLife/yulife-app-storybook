...
Feature("As a user I can navigate through member routes correctly", async () => {
  Scenario("I can view the core screens of the app", scenario.start, async () => {
    ...
    When("I tap quests in the tab", when.tapID(ids.NAV_BAR("quests"), 3000), async () => {
      Then("I should be on the quests screen", then.idVisible(ids.QUESTS_SCREEN(0)));
    });
    When("I tap leaderboard", when.tapID(ids.NAV_BAR("leaderboard"), 3000), async () => {
      Then("I should see that a leaderboard has not been set yet", then.checkCopyNoLeaderboard);
    });
    When("I tap rewards", when.tapID(ids.NAV_BAR("rewards"), 5000), async () => {
      When("I confirm my location", when.tapID(ids.REWARDS_LOCATION_CONFIRM, 10000), async () => {
  ...
  Scenario("I can view the menu screens in the app", scenario.start, async () => {
    ...
    When("I refresh the activity history page", when.tapID(ids.LEFT_HEADING_BUTTON("Activity history"), 2000), async () => {
      ...
      Then("I should see 14 mindful mins", then.idVisible(ids.ACTIVITY_HISTORY_CHALLENGE_VALUE("14 Mindful mins"), 1500));
  ...
  Scenario("I can talk to yulife via intercom", scenario.start, async () => {
    Given("Notifications permission is denied", given.denyNotifications, async () => {
      Given("I login as a user", given.loginAsUser(data.CUSTOMER_1, data.AUTH_1), async () => {
        Then("I should see a menu icon in the top left", then.idVisible(ids.MENU_ICON, 1500));
      });
    });
    ...
    When("I tap chat", when.tapMenuItem("Chat"), async () => {
      Then("I should see the notification request, as I have not allowed this permission", then.idVisible(ids.GENERIC_SCREEN_HEADING("notification"), 3000));
    });
    When("I tap skip", when.tapID(ids.GENERIC_SCREEN_CTA("skip"), 4000), async () => {
      Then("I should see intercom", then.textVisible("Send us a message", 1500));
    });
  });
