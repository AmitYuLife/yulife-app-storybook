...
import { navigation, navigateViaID } from "@utils"
...
export const tapYuniverseLevelForFirstTime = (questScreen: number, xCoordinate: number, yCoordinate: number, yCordinateIphoneSe?: number) => async () => {
    ...
    if (device.name.includes("(iPhone SE (3rd generation))") && yCordinateIphoneSe) {
        await element(by.id(ids.QUESTS_SCREEN_YUNIVERSAL(questScreen))).tap({x: xCoordinate, y: yCordinateIphoneSe});
    } else {
        await element(by.id(ids.QUESTS_SCREEN_YUNIVERSAL(questScreen))).tap({x: xCoordinate, y: yCoordinate});
    }
}
