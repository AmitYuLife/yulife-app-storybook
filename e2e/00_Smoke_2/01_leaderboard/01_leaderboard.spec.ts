...
Feature("As a user I can see my achievements on the leaderboard", async () => {
        ...
        Given("I login", given.loginAsUser(CUSTOMER_16, AUTH_16), async () => {
            When("I go to the leaderboard screen", when.tapID(ids.NAV_BAR("leaderboard")), async () => {
                ...
                When("I tap 'Yes'", when.tapText("Yes"), async () => {
                    Then("I should see the leaderboard title", then.idVisible(ids.LEADERBOARD_TITLE(LEADERBOARD_1.data.name)))
    ...
    Scenario("I can change consent to a leaderboard I belong to", scenario.start, async () => {
        Given("I login", given.loginAsUser(CUSTOMER_17, AUTH_17), async () => {
            When("I go to the leaderboard", when.tapID(ids.NAV_BAR("leaderboard")), async () => {
                ...
                When("I tap on the leaderboard name", when.tapID(ids.LEADERBOARD_TITLE(LEADERBOARD_1.data.name), 2000), async () => {
                    Then("I should see my active leaderboard", then.leaderboardStatus(LEADERBOARD_1.data.name, "active"))
                    When("I turn off this leaderboard", when.turnOffLeaderboard(ids.LEADERBOARD_SWITCH(LEADERBOARD_1.data.name)), async () => {
            ...
            When("I go to the leaderboard", when.tapID(ids.NAV_BAR("leaderboard")), async () => {
                When("I tap the dropdown", when.tapID(ids.LEADERBOARD_DROPDOWN), async () => {
                    When("I click leaderboard 2", when.tapText(LEADERBOARD_2.data.name), async () => {
                        Then("I should see the leaderboard title", then.idVisible(ids.LEADERBOARD_TITLE(LEADERBOARD_2.data.name)))
                        Then("I should be on the second leaderboard", then.leaderboardVisible([CUSTOMER_20]))
                    })
                })
            })
        })
        When("I scroll to the top of the page", when.scrollUntilIdVisible(ids.LEADERBOARD_SCROLL_LIST, ids.LEADERBOARD_TITLE(LEADERBOARD_2.data.name), "down"), async () => {
            When("I tap the leaderboard drop down", when.tapID(ids.LEADERBOARD_TITLE(LEADERBOARD_2.data.name), 2000), async () => {
    ...
    Scenario("I can inspect other members and view their data and avatars from the leaderboard - seed data", scenario.start, async () => {
        ...
        When("I click on user Michael's name", when.clickUser(CUSTOMER_18.data.fullName), async () => {
            ...
            Then("Michael's name is visible", then.idVisible(ids.TEXT_TEMPLATE(CUSTOMER_18.data.fullName))) 
            Then("Michael's world is visible", then.idVisible(ids.USER_WORLD("Forest")))
            Then("Michael's level is visible", then.idVisible(ids.USER_LEVEL(COIN_LEDGER_18.data.currentLevel)))
    ...
    Scenario("I can inspect myself and view my data and avatar from the leaderboard - seed data", scenario.start, async () => {
        ...
        When("I click on my own name", when.tapID(ids.RANK(CUSTOMER_47.data.fullName, 2)), async () => {
            Then("I should be on the Inspect screen", then.isOnInspectScreen)
            Then("My name is visible", then.idVisible(ids.TEXT_TEMPLATE(CUSTOMER_47.data.fullName))) 
            Then("My world is visible", then.idVisible(ids.USER_WORLD("Forest")))
            Then("My level is visible", then.idVisible(ids.USER_LEVEL(COIN_LEDGER_47.data.currentLevel)))
