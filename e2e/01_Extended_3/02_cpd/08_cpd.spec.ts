...
Feature("CPD/Yuniversity", async () => {
    Scenario("As a user with access to the Yuniversity, I should be able to see the various modules and chapters", scenario.start, () => {
        Given("I login", given.logInAndGoToTab("yucoin", CUSTOMER_20, AUTH_20), async () => {
            Then("I should see 220 YuCoin in the top right hand corner", then.idVisible(VIEW_TOP_RIGHT_COIN_COUNTER(220)))
        ...
        When("I tap on Yuniversity", when.tapText("Yuniversity"), async () => {
            Then("I should be able to see the CPD Courses screen", then.idVisible(CPD_COURSES_SCREEN))
            Then("I should see the details for Course 1", then.canSeeCourseDetails(YUNIVERSITY_COURSE_1))
            Then("I should see should be able to see the thumbnail for module 1 with the correct details", then.canSeeModuleThumbnail(YUNIVERSITY_COURSE_MODULE_1, 0))
            Then("I should see should be able to see the thumbnail for module 2 with the correct details", then.canSeeModuleThumbnail(YUNIVERSITY_COURSE_MODULE_1, 1))
        })
        When("I click to leave the CPD page", when.tapID(BACK_BUTTON), async () => {
            Then("I should be back on the Wellbeing Hub screen", then.idVisible(WELLBEING_HUB_SCREEN))
        })
        When("I tap on Yuniversity", when.tapText("Yuniversity"), async () => {
            Then("I should be able to see the CPD Courses screen", then.idVisible(CPD_COURSES_SCREEN))
        })
        When("I tap on Module One", when.tapModule(YUNIVERSITY_COURSE_MODULE_1), async () => {
            Then("I'm on the course detail screen", then.idVisible(CPD_COURSE_DETAIL_SCREEN))
            Then("I can see the details for Module 1", then.canSeeModuleDetails(YUNIVERSITY_COURSE_MODULE_1, cpdModule1Copy))
        })
        When("I scroll down the screen to see the chapters", when.scrollToChapter(YUNIVERSITY_COURSE_MODULE_1, 3, "down"), async () => {
            Then("I can see the thumbnail for Chapter 1", then.canSeeChapterThumbnail(YUNIVERSITY_COURSE_MODULE_1, 1))
            Then("I can see the thumbnail for Chapter 2", then.canSeeChapterThumbnail(YUNIVERSITY_COURSE_MODULE_1, 2))
            Then("I can see the thumbnail for Chapter 3", then.canSeeChapterThumbnail(YUNIVERSITY_COURSE_MODULE_1, 3))
        })
        When("I swipe down the screen", when.scrollUntilTextVisible(CPD_COURSE_SCROLL_VIEW, "Certificate", "down"), async () => {
        ...
        When("I swipe up the screen to see the chapters", when.scrollToChapter(YUNIVERSITY_COURSE_MODULE_1, 1, "up"), async () => {
            Then("I can see the thumbnail for Chapter 1", then.canSeeChapterThumbnail(YUNIVERSITY_COURSE_MODULE_1, 1))
        })
        When("I tap on Chapter 1", when.tapChapter(YUNIVERSITY_COURSE_MODULE_1, 1), async () => {
            Then("I wait until the 15 second test video has played", then.wait(15000))
            Then("I can see that chapter has been marked 'Completed'", then.chapterComplete(1))
        })
        When("I tap on Chapter 2", when.tapChapter(YUNIVERSITY_COURSE_MODULE_1, 2), async () => {
            Then("I wait until the 15 second test video has played", then.wait(15000))
            Then("I can see that chapter has been marked 'Completed'", then.chapterComplete(2))
        })
        When("I tap on Chapter 3", when.tapChapter(YUNIVERSITY_COURSE_MODULE_1, 3), async () => {
            Then("I wait until the 15 second test video has played", then.wait(15000))
        ...
        When("I swipe down the screen to the bottom", when.swipeFromText("Module notes", "up", "slow", 0.4), async () => {
            When("I tap on 'Take the quiz'", when.tapText("Take the quiz"), async () => {
                Then("I should be on the 'Take the quiz' page", then.canSeeQuizPage(YUNIVERSITY_COURSE_MODULE_1))
            })
        })
        ...
        When("I tap 'Done'", when.tapText("Done"), async () => {
            Then("I should be back on the course detail screen", then.idVisible(CPD_COURSE_DETAIL_SCREEN))
        ...
        When("I scroll to the bottom of the screen", when.swipeFromText("Take the quiz", "up", "slow"), async () => {
            When("I tap to view the certificate'", when.tapTextAtIndex("View", 1), async () => {
                Then("I should be be able to see the certificate", then.idVisible(CPD_CERTIFICATE))
                Then("I can see the details on the certificate are correct", then.canSeeCertificateDetails(YUNIVERSITY_COURSE_MODULE_1, CUSTOMER_20, BUSINESS_ACCOUNT_2))
            })
        })
        When("I swipe down", when.swipeFromText("CPD Module Certificate", "up", "slow"), async () => {
            Then("I should be able to see the 'Save as image' button", then.idVisible(CPD_SAVE_BUTTON))
        })
        When("I tap the close button", when.closeScreen("button_only"), async () => {
            When("I tap the close button", when.closeScreen("yulife"), async () => {
                When("tap the back button", when.tapID(BACK_BUTTON), async () => {
                    When("I go to settings", when.tapID(MENU_ICON), async () => {
                        When("I tap on 'Activity History'", when.tapID(MENU_ITEM("Activity History")), async () => {
                            Then("I should be on the Activity history screen", then.idVisible(ACTIVITY_HISTORY_SCREEN))
                        })
                    })
                })
            })
        ...
        When("I pull down the activity history page to refresh", when.swipeFromText(twoDaysAgoDate, "down", "fast"), async () => {
            When("I tap the close button", when.closeScreen("activity history"), async () => {
                Then("I should now have 420 YuCoin in the top right hand corner", then.idVisible(VIEW_TOP_RIGHT_COIN_COUNTER(420)))
