import * as bp from "./business_product";

const type = "postgres"
const modelName = "business_product_category"


export const BPC_USA_1_WB = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_1_WB.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_1_DENPPO = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_1_DENPPO.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_1_DENHMO = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_1_DENHMO.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_1_DENCHOI = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_1_DENCHOI.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_1_VIS = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_1_VIS.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_1_TLIF = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_1_TLIF.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_2_ADD = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_2_ADD.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_2_VADD = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_2_VADD.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_2_STD = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_2_STD.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_2_LTD = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_2_LTD.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_2_HI = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_2_HI.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_3_VSTD = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_3_VSTD.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_3_VLTD = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_3_VLTD.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_3_ACC = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_3_ACC.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_3_CRI = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_3_CRI.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_3_SPDIS = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_3_SPDIS.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_4_ACCSICK = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_4_ACCSICK.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_4_CAN = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_4_CAN.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_4_VLIF = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_4_VLIF.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_5_GAP = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_5_GAP.product.data.product_id,
        category_id: "1",
        earn_rate: 0,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
        "enrolment_method" : "external",
        "election_option" : "opt-in",
    },
};

export const BPC_USA_5_VIS = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_5_VIS.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_6_GAP = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_6_GAP.product.data.product_id,
        category_id: "1",
        earn_rate: 0,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
        "enrolment_method" : "external",
        "election_option" : "opt-in",
    },
};

export const BPC_USA_7_VIS = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_7_VIS.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_8_GAP = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_8_GAP.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_10_GAP = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_10_GAP.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_10_VIS = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_10_VIS.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_11_ACC = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_11_ACC.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_11_CAN = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_11_CAN.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_12_CAN = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_12_CAN.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};


export const BPC_USA_13_CRI = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_13_CRI.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_14_VSTD = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_14_VSTD.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};

export const BPC_USA_15_CRI = {
    type,
    modelName,
    data:{
        product_id: bp.BUSINESS_PRODUCT_USA_15_CRI.product.data.product_id,
        category_id: "1",
        earn_rate: 17,
        category_name: "App access only",
        category_description: "All employees will have access to the YuLife app",
    },
};