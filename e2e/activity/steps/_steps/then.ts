...
export const onLeaderboardConsent = async () => {
  const firstParagraph =
    "By joining the leaderboard, you are consenting to share details about your activity with other members of this leaderboard.";
  const secondParagraph =
    "You can opt out at any time by tapping the name of the leaderboard and adjusting your settings.";
...
export const canSeeHistoricalSteps = (days: number, steps: number) => async () => {
  for (let i = 0; i < days; i++) {
    await scrollUntilIdVisibleAtIndex(
      ids.ACTIVITY_HISTORY_SCREEN,
      ids.ACTIVITY_HISTORY_CHALLENGE_VALUE(`${formatNumber(steps, 0)} Steps`),
      "up",
      0
    )();
  }
