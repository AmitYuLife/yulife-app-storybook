...
Feature("As a user my cycling distance is monitored correctly", async () => {
  Scenario("I can take and complete a cycling event and hit all the event milestones", scenario.start, async () => {
    Given("I login", given.loginAsUser(data.CUSTOMER_71, data.AUTH_71), async () => {
    ...
    When("I click Claim", when.tapID(ids.COLLECT_EVENT_REWARD_BUTTON, 5000), async () => {
      Then("I should see the first milestone complete", then.milestoneComplete(0));
    });
    When("I click the back button", when.tapID(ids.BACK_BUTTON, 4000), async () => {
    ...
    When("I have done 3 km today", when.addCyclingData(3000), async () => {
      When("I update the screen to see today's activity pulled through", given.triggerAppUpdateState, async () => {
        When("I go back to the yucoin tab", when.tapID(ids.NAV_BAR("yucoin"), 10_000), async () => {
    ...
    When("I click Claim", when.tapID(ids.EVENT_DIALOG_BUTTON, 4000), async () => {
      Then("I should be on the event milestone page", then.onCompletedEventMilestonePage("100", 2));
    });
    When("I click Claim", when.tapID(ids.COLLECT_EVENT_REWARD_BUTTON, 4000), async () => {
    ...
    When("I have done 5 km today", when.addCyclingData(5000), async () => {
      When("I update the screen to see today's activity pulled through", given.triggerAppUpdateState, async () => {
        When("I go back to the yucoin tab", when.tapID(ids.NAV_BAR("yucoin"), 10_000), async () => {
          Then("I should see 10 km cycled", then.idVisible(ids.CYCLING_COUNT("10.0 km"), 3000));
          Then("I should see I have earned 30 YuCoin from the cycle", then.textVisible(t(`1,760 ${t("YuCoin")} ${t("today")}`), 2000));
        });
      });
    });
    ...
    When("I click Great! button", when.tapID(ids.COLLECT_EVENT_REWARD_BUTTON, 3000), async () => {
    ...
    When("I click the back button", when.tapID(ids.BACK_BUTTON, 3000), async () => {
  ...
  Scenario("Cycling data is successfully synced on log in through today's activities query", scenario.start, async () => {
    Given("I login and go to Yucoin screen", given.loginAsUser(data.CUSTOMER_73, data.AUTH_73), async () => {
