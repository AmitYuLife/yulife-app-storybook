...
export const onLeaderboardWithoutConsent =
  (steps = true, yudoku = true) =>
  async () => {
...
export const canSeeLeaderboardListModal =
  (socialGroupLeaderboards: SocialGroupLeaderboard[], consent: boolean) => async () => {
...
export const leaderboardVisible =
  (customers: UserLeaderboardListItem[], waitTime?: number) => async () => {
    await wait(waitTime)();
...
export const leaderboardStatus =
  (leaderboardName: string, status: "active" | "inactive") => async () => {
    await wait(2000)();
...
export const comparativeUserStatsVisible =
  (opponentAvSteps: number, myAvSteps: number) => async () => {
...
export const comparativeUserSeedStatsVisible =
  (opponentAvSteps: number, myAvSteps: number) => async () => {
    await scrollFromID(ids.COMPARISON_STATS_SECTION, "up", "fast", 0.6)();
...
export const comparativeUserCyclingMindfulnessStats =
  (oppAvKm: number, myAvKm: number, oppAvMins: number, myAvMins: number) => async () => {
...
export const onChallengeAFriend = async () => {
  ...
  await textVisible(
    "Want to go head-to-head? Select a friend from the leaderboard, and challenge them to a duel!"
  )();
};
...
export const isOnOutStepOpponentScreen = async () => {
  ...
  await textVisible(
    "Your duel will begin the next day and you’ll have 24 hours to get in as many steps as you can!"
  )();
};
...
export const minsComparativeDrawResults =
  (opponentAvMinduflness: number, myAvMindfulness: number) => async () => {
    await idVisible(ids.COMPARISON_ACTIVITY("Average mindfulness"), 1000)();
...
export const cyclingComparativeDrawResults =
  (opponentAvCycling: number, myAvCycling: number) => async () => {
    await idVisible(ids.COMPARISON_ACTIVITY("Average mindfulness"), 1000)();
