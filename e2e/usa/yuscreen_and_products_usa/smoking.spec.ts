...
Feature("As a user from USA, I can view and use the smoking cessation feature", async () => {
  Scenario("I can begin my smoking cessation journey, fill out the questionnaire, and track view the smoking hub", scenario.start, async () => {
    Given("I log in and go to the YuScreen", given.logInAndGoToTab("yu", data.CUSTOMER_USA_1, data.AUTH_USA_1, true, "US"), async () => {
      ...
      Then("I can see the YuScreen scroll view", then.idVisible(ids.YUSCREEN_SCROLL_VIEW));
    });
    When("I swipe up to scroll down", when.scrollWithLimitedAttemptsUntilIdVisible(ids.YUSCREEN_SCROLL_VIEW, ids.YUSCREEN_SMOKING_TILE, "up"), async () => {
      Then("I should see the smoking tile", then.idVisible(ids.YUSCREEN_SMOKING_TILE));
    });
    ...
    When("I tap start my journey", when.tapID(ids.BUTTON_BASE("Start my journey")), async () => {
    ...
    When("I tap the 'I’m committed' button", when.tapID(ids.BUTTON_BASE("I’m committed")), async () => {
    ...
    When("I tap the 'Take your first steps' button", when.tapID(ids.BUTTON_BASE("Take your first steps")), async () => {
      Then("I should be able to tap to move forward", then.idVisible(ids.FULL_SCREEN_SWIPER_TOUCH_CONTROLLER));
      When("I tap the touch controller", when.tapID(ids.FULL_SCREEN_SWIPER_TOUCH_CONTROLLER), async () => {
        Then("I should still be in the Story viewing screen", then.idVisible(ids.FULL_SCREEN_SWIPER_TOUCH_CONTROLLER));
        When("I tap the page for a second time", when.tapID(ids.FULL_SCREEN_SWIPER_TOUCH_CONTROLLER), async () => {
    ...
    When("I close and reopen the app", when.minimiseAndReopenApp, async () => {
      ...
      When("I wait", when.wait(30000), async () => {
        Then("I can still see a way to go back", then.idVisible(ids.LEFT_HEADING_BUTTON("TODAY’S_EARNINGS")));
        When("I go back from the Today's Earnings screen", when.tapID(ids.LEFT_HEADING_BUTTON("TODAY’S_EARNINGS")), async () => {
    ...
    When("I tap the smoking card", when.tapID(ids.EVENT_CARD("Track your progress")), async () => {
