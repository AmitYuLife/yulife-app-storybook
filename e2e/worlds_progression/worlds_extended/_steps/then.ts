...
export const { scrollFromText } = navigation.scrolling;

export const { unlockedYumojiItemsVisible } = screens.yuscreen;

export const onChallengeHistory =
  (
  ...
  async () => {
    ...
    for (const yucoin of yucoinNums) {
      const str = `${yucoin}`;
      const id = ids.CHALLENGE_HISTORY_YUCOIN_STARS(
        str,
        starsCount[yuCoinIndex],
        challengeType,
        yuCoinIndex
      );
...
export const isOnExploreYuniverseScreen = async () => {
  await textVisible(
    "With your first Yuniversal journey complete, now is a time for reflection and gratitude as you drift amongst the stars. Familiar friends will guide you on your path towards a celestial chest, and what’s inside ...",
    3000
  );
  await navigateViaText("Explore the Yuniverse");
};

export const isOnChestScreen = (worldType: WorldType) => async () => {
  await wait(5000)();
  let label = "";
  let subheading = "";
