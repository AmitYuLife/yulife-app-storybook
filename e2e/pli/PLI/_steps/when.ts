 ...
 */
export const swipeOnPicker =
  (scrollViewId: string, id: string, direction: Detox.Direction, speed = 150) =>
  async () => {
    ...
    while (!isVisible) {
      await scroller.scroll(speed, direction, 0.5, 0.5);
      await wait(500)();
...
export const addGPDetails = async () => {
  ...
  const reportText =
    "Tick here if you want to see your medical report before your doctor sends it to us. Please note by checking this option you will be invited to your GP surgery to review your report.";
  const acceptStatement = "I have read and agreed to the above statements.";
  const acceptAndRead =
    "I understand that the monthly price may be subject to change once my medical information has been assessed and that I will receive a final price before I commit.";
...
export const addPaymentDetails = async () => {
  ...
  // DO NOT ADD `\n` here as the card details is a native modal and we dont want to hide the keyboard
