...
Feature("As a user I can opt in and take an event", async () => {
  Scenario("I can take and complete a 3 star challenge event and hit all the event milestones, with the daily hero card toggle", scenario.start, async () => {
    ...
    When("I scroll down the challenge list", when.scrollFromID(ids.CHALLENGE_TILE("Long Walk"), "down", "fast", 0.3, 1_000), async () => {
      When("I complete a short stroll challenge", when.selectAndCompleteWalkingChallenge("Short Stroll", 400), async () => {
        Then("I should see the correct challenge and award details on the screen", then.stepsChallengeDataCorrect(152, 40, 400, true));
      });
    });
    When("I tap collect", when.tapID(ids.CHALLENGE_SUCCESS_CTA, 2000), async () => {
    ...
    When("I complete a brisk walk challenge", when.selectAndCompleteWalkingChallenge("Brisk Walk", 1200), async () => {
      Then("I should see the correct challenge and award details on the screen", then.stepsChallengeDataCorrect(152, 60, 1200 + 400, true));
    });
    When("I tap collect", when.tapID(ids.CHALLENGE_SUCCESS_CTA, 2000), async () => {
    ...
    When("I complete a long walk challenge", when.selectAndCompleteWalkingChallenge("Long Walk", 3000), async () => {
      Then("I should see the correct challenge and award details on the screen", then.stepsChallengeDataCorrect(152, 80, 1600 + 3000, true));
    });
    When("I tap collect", when.tapID(ids.CHALLENGE_SUCCESS_CTA, 2000), async () => {
  ...
  Scenario("Passive data is correctly logged for an event that has backfill enabled, and the event panel should reflect the world I am on", scenario.start, async () => {
    ...
    When("I have done 4,000 steps yesterday", when.addStepsHistoricalData(4000), async () => {
      Then("I should be on the yucoin screen", then.idVisible(ids.DAILY_STEPS_SCREEN, 3000));
    });
    ...
    // @update -- Temp Skip as event shows no progress for yesterday passive steps
