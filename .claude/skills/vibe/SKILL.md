...
0. **Pre-flight: check `src/graphql/_core/client.ts` is NOT pointing at a remote host.** Grep `buildRegionalGqlUri` in that file. The correct form is `${region.getRegionUri(r)}/graphql` — anything else (hardcoded `api.develop.uk.yulife.engineering`, `api.staging.…`, etc.) means a previous debug session left a hot-patch in place. If you find one, restore it to `${region.getRegionUri(r)}/graphql` before doing anything else. A hardcoded remote URL is the single most expensive failure mode in this skill: login works (vibes/utils/login.ts uses axios directly to localhost), but every subsequent Apollo call hits the remote dev server, gets `Unauthenticated`, and `expireSession()` kicks the user back to the login screen — looks identical to a token-injection bug and burns hours.
